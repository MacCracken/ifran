...
pub async fn execute(
    ...
    let http_client = ifran_core::pull::downloader::build_client()?;
    let infer_fn = |prompt: String| {
        ...
        async move {
            ...
            if !resp.status().is_success() {
                return Err(ifran_types::IfranError::EvalError(format!(
            ...
            let body: serde_json::Value = resp.json().await.map_err(|e| {
                ifran_types::IfranError::EvalError(format!("Invalid inference response: {e}"))
            })?;
...
fn parse_benchmark(s: &str) -> ifran_types::error::Result<BenchmarkKind> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(ifran_types::IfranError::EvalError(format!(
