//! AMD XDNA backend integration.
//!
//! HTTP client to a running Vitis AI / ONNX Runtime XDNA serving process
//! for AMD Ryzen AI NPUs. It exposes an OpenAI-compatible API that this
//! backend wraps.
...
/// AMD XDNA backend that proxies to a running Vitis AI / ONNX Runtime XDNA server.
...
impl XdnaBackend {
    pub fn new(base_url: Option<String>) -> Self {
        Self {
            base_url: base_url.unwrap_or_else(|| "http://127.0.0.1:8008".into()),
...
#[async_trait]
impl InferenceBackend for XdnaBackend {
    ...
    fn capabilities(&self) -> BackendCapabilities {
        BackendCapabilities {
            accelerators: vec![AcceleratorType::AmdXdna],
    ...
    async fn load_model(
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| IfranError::BackendError(format!("Cannot reach XDNA server: {e}")))?;

        if !resp.status().is_success() {
            return Err(IfranError::BackendError(
                "XDNA server returned error on /v1/models".into(),
            ));
        }

        let handle_id = format!("xdna-{}", model_name.replace('/', "-"));
        info!(handle = %handle_id, model = %model_name, "Registered model with XDNA backend");
    ...
    async fn unload_model(&self, handle: ModelHandle) -> Result<()> {
        ...
        if loaded.remove(&handle.0).is_some() {
            info!(handle = %handle.0, "Unregistered model from XDNA backend");
            Ok(())
        } else {
    ...
    async fn infer(
        ...
        if !resp.status().is_success() {
            ...
            return Err(IfranError::BackendError(format!("XDNA error: {text}")));
        }
        ...
        tokio::spawn(async move {
            ...
            while let Some(chunk) = stream.next().await {
                ...
                let chunk = match chunk {
                    Ok(c) => c,
                    Err(e) => {
                        warn!("XDNA stream error: {e}");
                        break;
                    }
                };
                buffer.push_str(&String::from_utf8_lossy(&chunk));
                if buffer.len() > MAX_BUFFER_SIZE {
                    warn!("XDNA stream buffer exceeded {MAX_BUFFER_SIZE} bytes, aborting");
                    break;
                }
...
mod tests {
    ...
    #[test]
    fn backend_id() {
        let backend = XdnaBackend::new(None);
        assert_eq!(backend.id().0, "xdna");
    }

    #[test]
    fn default_url() {
        let backend = XdnaBackend::new(None);
        assert_eq!(backend.base_url, "http://127.0.0.1:8008");
    }

    #[test]
    fn custom_url() {
        let backend = XdnaBackend::new(Some("http://npu-server:8008".into()));
        assert_eq!(backend.base_url, "http://npu-server:8008");
    }
    ...
    fn capabilities() {
        ...
        assert!(caps.accelerators.contains(&AcceleratorType::AmdXdna));
    ...
    async fn load_model_success() {
        ...
        let backend = XdnaBackend::new(Some(server.url()));
        ...
        let device = DeviceConfig {
            accelerator: AcceleratorType::AmdXdna,
            device_ids: vec![0],
            memory_limit_mb: None,
        };

        let handle = backend.load_model(&manifest, &device).await.unwrap();
        assert_eq!(handle.0, "xdna-org-test-model");
    ...
    async fn unload_model_success() {
        ...
        backend
            ...
            .write()
            .await
            .insert("xdna-test".into(), "test-model".into());

        backend
            .unload_model(ModelHandle("xdna-test".into()))
            .await
            .unwrap();
    ...
    async fn infer_success() {
        ...
        let resp = backend
            .infer(&ModelHandle("xdna-test".into()), &req)
            .await
            .unwrap();
    ...
    async fn infer_server_error() {
        ...
        backend
            ...
            .write()
            .await
            .insert("xdna-test".into(), "model".into());
        ...
        let result = backend.infer(&ModelHandle("xdna-test".into()), &req).await;
        assert!(matches!(result, Err(IfranError::BackendError(_))));
        mock.assert_async().await;
    }
    ...
    async fn health_check_server_down() {
        let backend = XdnaBackend::new(Some("http://127.0.0.1:1".into()));
        let healthy = backend.health_check().await.unwrap();
        assert!(!healthy);
    }
}
