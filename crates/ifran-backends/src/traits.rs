...
pub trait InferenceBackend: Send + Sync {
    ...
    async fn load_model(
        ...
        device: &DeviceConfig,
    ) -> ifran_types::error::Result<ModelHandle>;
    async fn unload_model(&self, handle: ModelHandle) -> ifran_types::error::Result<()>;
    async fn infer(
        &self,
        handle: &ModelHandle,
        req: &InferenceRequest,
    ) -> ifran_types::error::Result<InferenceResponse>;
    async fn infer_stream(
        &self,
        handle: &ModelHandle,
        req: InferenceRequest,
    ) -> ifran_types::error::Result<mpsc::Receiver<StreamChunk>>;
    async fn health_check(&self) -> ifran_types::error::Result<bool>;
    ...
    async fn embed(
        ...
        text: &str,
        dims: usize,
    ) -> ifran_types::error::Result<Vec<f32>> {
