...
impl BridgeClient {
    ...
    pub async fn reconnect(&self) -> ifran_types::error::Result<()> {
        ...
        if *count >= self.config.max_reconnect_attempts {
            ...
            *self.state.write().await = ConnectionState::Degraded;
            return Err(ifran_types::IfranError::BridgeError(
    ...
    pub async fn announce(&self, capabilities: Capabilities) -> ifran_types::error::Result<()> {
        info!(
    ...
    pub async fn request_gpu(
        ...
        memory_mb: u64,
        count: u32,
    ) -> ifran_types::error::Result<Vec<u32>> {
    ...
    pub async fn request_scale_out(
        ...
        additional: u32,
        reason: &str,
    ) -> ifran_types::error::Result<ScaleResponse> {
        ...
        if let Some(mut client) = self.get_client().await? {
            ...
            let response = client.request_scale_out(request).await.map_err(|e| {
                ifran_types::IfranError::BridgeError(format!("RequestScaleOut RPC failed: {e}"))
            })?;
