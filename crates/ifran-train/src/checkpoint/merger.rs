...
base_model = os.environ["IFRAN_MERGE_BASE_MODEL"]
adapter_path = os.environ["IFRAN_MERGE_ADAPTER_PATH"]
output_path = os.environ["IFRAN_MERGE_OUTPUT_PATH"]
...
"#,
        ...
        .env("IFRAN_MERGE_BASE_MODEL", base_model)
        .env("IFRAN_MERGE_ADAPTER_PATH", adapter_path)
        .env("IFRAN_MERGE_OUTPUT_PATH", output_path)
        .output()
        .await
        .map_err(|e| IfranError::TrainingError(format!("Failed to run merge script: {e}")))?;
