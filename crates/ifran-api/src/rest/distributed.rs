...
pub struct AutoPlaceRequest {
    ...
    pub policy: ifran_types::distributed::PlacementPolicyKind,
    /// Optional: GPUs per worker (default: 1).
    #[serde(default = "default_gpus_per_worker")]
    pub gpus_per_worker: u32,
}

fn default_placement_policy() -> ifran_types::distributed::PlacementPolicyKind {
    ifran_types::distributed::PlacementPolicyKind::GpuAffinity
}
...
pub async fn aggregate(
    ...
    use ifran_train::distributed::aggregator::{
...
pub async fn auto_place(
    ...
    let online_nodes: Vec<_> = fleet_nodes
        .iter()
        .filter(|n| n.health == ifran_core::fleet::manager::NodeHealth::Online)
        .collect();
    ...
    let node_resources: Vec<ifran_train::distributed::placement::NodeResources> = online_nodes
        .iter()
        .map(|n| {
            let available_gpu_ids: Vec<u32> = (0..n.gpu_info.gpu_count as u32).collect();
            ifran_train::distributed::placement::NodeResources {
                node_id: n.id.clone(),
    ...
    let policy = ifran_train::distributed::placement::policy_from_kind(req.policy);
