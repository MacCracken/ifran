...
pub async fn create_auto_label(
    ...
    let config = ifran_train::dataset::labeler::AutoLabelConfig {
    ...
    tokio::spawn(async move {
        ...
        let infer_fn = |prompt: String| {
            ...
            async move {
                ...
                let loaded_model = loaded
                    ...
                    .ok_or_else(|| {
                        ifran_types::IfranError::TrainingError(
                            "No model loaded for auto-labeling".into(),
                        )
                    })?;

                let backend_id = ifran_types::backend::BackendId(loaded_model.backend_id.clone());
                let backend = backends.get(&backend_id).ok_or_else(|| {
                    ifran_types::IfranError::TrainingError(format!(
        ...
        match ifran_train::dataset::labeler::run_labeling(
            &label_config,
            &output_path,
            &labeler,
            job_id,
...
fn job_to_auto_label_response(
    job: &ifran_train::dataset::labeler::AutoLabelJobState,
...
pub async fn augment_dataset(
    ...
    let config = ifran_train::dataset::processor::AugmentationConfig {
    ...
    let result = tokio::task::spawn_blocking(move || {
        ifran_train::dataset::processor::augment_dataset(
...
pub struct ValidateDatasetRequest {
    pub path: String,
    pub format: ifran_types::training::DatasetFormat,
}
...
pub async fn validate_dataset(
    ...
    let result = tokio::task::spawn_blocking(move || {
        ifran_train::dataset::validator::validate(std::path::Path::new(&path), format)
    })
    .await
    .map_err(|e| {
        (
...
pub async fn preview_dataset(
    ...
    let result =
        tokio::task::spawn_blocking(move || -> Result<(Vec<serde_json::Value>, bool), String> {
            ...
            match format {
                ifran_types::training::DatasetFormat::Jsonl => {
                ...
                ifran_types::training::DatasetFormat::Csv => {
...
mod tests {
    ...
    fn validate_request_deserialize() {
        ...
        assert_eq!(req.format, ifran_types::training::DatasetFormat::Jsonl);
    }
    ...
    fn validate_request_csv_format() {
        ...
        let req: ValidateDatasetRequest = serde_json::from_str(json).unwrap();
        assert_eq!(req.format, ifran_types::training::DatasetFormat::Csv);
    }
    ...
    fn preview_response_serializes() {
        ...
        let resp = PreviewDatasetResponse {
            samples,
            has_more: true,
            format: ifran_types::training::DatasetFormat::Jsonl,
        };
        let json = serde_json::to_value(&resp).unwrap();
