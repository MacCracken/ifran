...
pub struct MetricsResponse {
    ...
    pub current_epoch: f32,
    pub progress_percent: f64,
    pub checkpoints: Vec<ifran_types::training::CheckpointInfo>,
}
...
async fn load_checkpoints(
    state: &AppState,
    id: TrainingJobId,
) -> Result<Vec<ifran_types::training::CheckpointInfo>, ApiErrorResponse> {
    let checkpoints_dir = state.config.training.checkpoints_dir.clone();
    tokio::task::spawn_blocking(move || {
        ifran_train::checkpoint::store::CheckpointStore::new(checkpoints_dir).list(id)
    })
    .await
...
pub async fn list_checkpoints(
    ...
    Extension(tenant_id): Extension<TenantId>,
    Path(id): Path<TrainingJobId>,
) -> Result<Json<Vec<ifran_types::training::CheckpointInfo>>, ApiErrorResponse> {
    state
        .job_manager
        .get_job(id, &tenant_id)
        .await
...
fn job_to_response(job: &ifran_train::job::status::JobState) -> JobResponse {
    JobResponse {
        id: job.id,
        status: job.status,
        current_step: job.current_step,
...
mod tests {
    ...
    async fn job_to_response_conversion() {
        ...
        let mut job =
            ifran_train::job::status::JobState::new(id, TenantId::default_tenant(), config, 1000);
        job.start();
