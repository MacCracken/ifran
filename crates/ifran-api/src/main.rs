...
#[tokio::main]
async fn main() {
    ifran_api::middleware::telemetry::init_tracing();
    ...
    if config.security.require_encrypted_storage {
        ifran_core::storage::encryption::verify_encryption_requirement(
    ...
    if bridge_enabled {
        ...
        if let Some(server) = &state.bridge_server {
            ...
            let grpc_service = ifran_bridge::server::IfranBridgeService::new(
            ...
            (state.bridge_server.clone(), state.bridge_client.clone())
        {
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    let gpu_free = ifran_core::hardware::detect::detect()
                        .ok()
    ...
    if state.config.fleet.enabled {
        let hardware = ifran_core::hardware::detect::detect().ok();
        ...
        let _ = state
            .fleet_manager
            .register(ifran_core::fleet::manager::RegisterNodeRequest {
                id: instance_id,
