...
pub fn verify_encryption_requirement(
    ...
    if require_encrypted && status == EncryptionStatus::Unencrypted {
        return Err(IfranError::ConfigError(format!(
...
pub async fn request_unlock(path: &Path, daimon_endpoint: &str) -> Result<()> {
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|e| IfranError::StorageError(format!("HTTP client error: {e}")))?;
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| IfranError::StorageError(format!("Daimon unlock request failed: {e}")))?;
    ...
    if resp.status().is_success() {
        ...
        Err(IfranError::StorageError(format!(
