...
impl HfClient {
    ...
    pub async fn model_info(&self, repo_id: &str) -> Result<HfModelInfo> {
        ...
        if resp.status() == reqwest::StatusCode::NOT_FOUND {
            return Err(IfranError::ModelNotFound(repo_id.to_string()));
        }
    ...
    pub async fn resolve_gguf(&self, repo_id: &str, quant_filter: Option<&str>) -> Result<HfFile> {
        ...
        if files.is_empty() {
            return Err(IfranError::ModelNotFound(format!(
        ...
        files
            .into_iter()
            .next()
            .ok_or_else(|| IfranError::ModelNotFound(format!("No GGUF files found in {repo_id}")))
    }
