...
impl InferenceBackend for TpuBackend {
    ...
    async fn load_model(
        ...
        let resp = self.client.get(&url).send().await.map_err(|e| {
            SynapseError::BackendError(format!("Cannot reach TPU serving process: {e}"))
        })?;
    ...
    async fn infer(
        ...
        if !resp.status().is_success() {
            ...
            return Err(SynapseError::BackendError(format!("TPU error: {text}")));
        }
        ...
        tokio::spawn(async move {
            ...
            while let Some(chunk) = stream.next().await {
                ...
                let chunk = match chunk {
                    Ok(c) => c,
                    Err(e) => {
                        warn!("TPU stream error: {e}");
                        break;
                    }
                };
                buffer.push_str(&String::from_utf8_lossy(&chunk));
                if buffer.len() > MAX_BUFFER_SIZE {
                    warn!("TPU stream buffer exceeded {MAX_BUFFER_SIZE} bytes, aborting");
                    break;
                }
