//! AWS Inferentia backend integration.
//!
//! HTTP client to a running transformers-neuronx or Neuron-based serving
//! process on AWS Inferentia/Trainium chips. It exposes an OpenAI-compatible
//! API that this backend wraps.
...
pub struct InferentiaBackend {
    /// Base URL of the Inferentia serving process.
    base_url: String,
    /// HTTP client.
...
impl InferentiaBackend {
    pub fn new(base_url: Option<String>) -> Self {
        Self {
            base_url: base_url.unwrap_or_else(|| "http://127.0.0.1:8005".into()),
...
#[async_trait]
impl InferenceBackend for InferentiaBackend {
    fn id(&self) -> BackendId {
        BackendId("inferentia".into())
    }

    fn capabilities(&self) -> BackendCapabilities {
        BackendCapabilities {
            accelerators: vec![AcceleratorType::Inferentia],
    ...
    async fn load_model(
        ...
        // Inferentia serving loads models at server startup. We register the
        ...
        let resp = self.client.get(&url).send().await.map_err(|e| {
            SynapseError::BackendError(format!("Cannot reach Inferentia server: {e}"))
        })?;

        if !resp.status().is_success() {
            return Err(SynapseError::BackendError(
                "Inferentia server returned error on /v1/models".into(),
            ));
        }

        let handle_id = format!("inferentia-{}", model_name.replace('/', "-"));
        info!(handle = %handle_id, model = %model_name, "Registered model with Inferentia backend");
    ...
    async fn unload_model(&self, handle: ModelHandle) -> Result<()> {
        ...
        if loaded.remove(&handle.0).is_some() {
            info!(handle = %handle.0, "Unregistered model from Inferentia backend");
            Ok(())
        } else {
...
mod tests {
    ...
    #[test]
    fn backend_id() {
        let backend = InferentiaBackend::new(None);
        assert_eq!(backend.id().0, "inferentia");
    }

    #[test]
    fn default_url() {
        let backend = InferentiaBackend::new(None);
        assert_eq!(backend.base_url, "http://127.0.0.1:8005");
    }

    #[test]
    fn custom_url() {
        let backend = InferentiaBackend::new(Some("http://inf-server:8005".into()));
        assert_eq!(backend.base_url, "http://inf-server:8005");
    }
    ...
    fn capabilities() {
        ...
        assert!(caps.accelerators.contains(&AcceleratorType::Inferentia));
    ...
    async fn load_model_success() {
        ...
        let backend = InferentiaBackend::new(Some(server.url()));
        ...
        let device = DeviceConfig {
            accelerator: AcceleratorType::Inferentia,
            device_ids: vec![0],
            memory_limit_mb: None,
        };

        let handle = backend.load_model(&manifest, &device).await.unwrap();
        assert_eq!(handle.0, "inferentia-org-test-model");
    ...
    async fn unload_model_success() {
        ...
        backend
            ...
            .write()
            .await
            .insert("inferentia-test".into(), "test-model".into());

        backend
            .unload_model(ModelHandle("inferentia-test".into()))
            .await
            .unwrap();
    ...
    async fn infer_success() {
        ...
        let resp = backend
            .infer(&ModelHandle("inferentia-test".into()), &req)
            .await
            .unwrap();
    ...
    async fn infer_server_error() {
        ...
        let mock = server
            .mock("POST", "/v1/chat/completions")
            .with_status(500)
            .with_body("NeuronCore out of memory")
            .create_async()
            .await;
        ...
        backend
            ...
            .write()
            .await
            .insert("inferentia-test".into(), "model".into());
    ...
    async fn health_check_server_down() {
        let backend = InferentiaBackend::new(Some("http://127.0.0.1:1".into()));
        let healthy = backend.health_check().await.unwrap();
        assert!(!healthy);
    }
}
