...
//!
//! HTTP client to an MLX-based serving process or llama.cpp with Metal
//! acceleration. Targets Apple Silicon GPUs via the Metal API and exposes
...
/// Metal backend that proxies to an MLX or llama.cpp Metal server.
pub struct MetalBackend {
    /// Base URL of the Metal serving process.
    base_url: String,
    /// HTTP client.
...
impl MetalBackend {
    pub fn new(base_url: Option<String>) -> Self {
        Self {
            base_url: base_url.unwrap_or_else(|| "http://127.0.0.1:8002".into()),
...
impl InferenceBackend for MetalBackend {
    fn id(&self) -> BackendId {
        BackendId("metal".into())
    }

    fn capabilities(&self) -> BackendCapabilities {
        BackendCapabilities {
            accelerators: vec![AcceleratorType::Metal],
    ...
    async fn load_model(
        ...
        let resp =
            self.client.get(&url).send().await.map_err(|e| {
                SynapseError::BackendError(format!("Cannot reach Metal server: {e}"))
            })?;

        if !resp.status().is_success() {
            return Err(SynapseError::BackendError(
                "Metal server returned error on /v1/models".into(),
            ));
        }

        let handle_id = format!("metal-{}", model_name.replace('/', "-"));
        info!(handle = %handle_id, model = %model_name, "Registered model with Metal backend");
    ...
    async fn unload_model(&self, handle: ModelHandle) -> Result<()> {
        ...
        if loaded.remove(&handle.0).is_some() {
            info!(handle = %handle.0, "Unregistered model from Metal backend");
            Ok(())
        } else {
    ...
    async fn infer(
        ...
        if !resp.status().is_success() {
            ...
            return Err(SynapseError::BackendError(format!("Metal error: {text}")));
        }
        ...
        tokio::spawn(async move {
            ...
            while let Some(chunk) = stream.next().await {
                ...
                let chunk = match chunk {
                    Ok(c) => c,
                    Err(e) => {
                        warn!("Metal stream error: {e}");
                        break;
                    }
                };
                buffer.push_str(&String::from_utf8_lossy(&chunk));
                if buffer.len() > MAX_BUFFER_SIZE {
                    warn!("Metal stream buffer exceeded {MAX_BUFFER_SIZE} bytes, aborting");
                    break;
                }
...
mod tests {
    ...
    #[test]
    fn backend_id() {
        let backend = MetalBackend::new(None);
        assert_eq!(backend.id().0, "metal");
    }

    #[test]
    fn default_url() {
        let backend = MetalBackend::new(None);
        assert_eq!(backend.base_url, "http://127.0.0.1:8002");
    }

    #[test]
    fn custom_url() {
        let backend = MetalBackend::new(Some("http://mac-mini:8002".into()));
        assert_eq!(backend.base_url, "http://mac-mini:8002");
    }
    ...
    async fn load_model_success() {
        ...
        let backend = MetalBackend::new(Some(server.url()));
        ...
        assert_eq!(handle.0, "metal-org-test-model");
    ...
    async fn unload_model_success() {
        ...
        backend
            ...
            .write()
            .await
            .insert("metal-test".into(), "test-model".into());

        backend
            .unload_model(ModelHandle("metal-test".into()))
            .await
            .unwrap();
    ...
    async fn infer_success() {
        ...
        let resp = backend
            .infer(&ModelHandle("metal-test".into()), &req)
            .await
            .unwrap();
    ...
    async fn infer_server_error() {
        ...
        backend
            ...
            .write()
            .await
            .insert("metal-test".into(), "model".into());
        ...
        let result = backend.infer(&ModelHandle("metal-test".into()), &req).await;
        assert!(matches!(result, Err(SynapseError::BackendError(_))));
        mock.assert_async().await;
    }
    ...
    async fn health_check_server_down() {
        let backend = MetalBackend::new(Some("http://127.0.0.1:1".into()));
        let healthy = backend.health_check().await.unwrap();
        assert!(!healthy);
    }
}
