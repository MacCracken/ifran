...
//!
//! HTTP client to a llama.cpp or Kompute-based Vulkan serving process.
//! Targets GPUs via the Vulkan API for cross-platform acceleration and
//! exposes an OpenAI-compatible HTTP interface that this backend wraps.
...
/// Vulkan backend that proxies to a llama.cpp or Kompute Vulkan server.
pub struct VulkanBackend {
    /// Base URL of the Vulkan serving process.
    base_url: String,
    /// HTTP client.
...
impl VulkanBackend {
    pub fn new(base_url: Option<String>) -> Self {
        Self {
            base_url: base_url.unwrap_or_else(|| "http://127.0.0.1:8003".into()),
...
impl InferenceBackend for VulkanBackend {
    fn id(&self) -> BackendId {
        BackendId("vulkan".into())
    }

    fn capabilities(&self) -> BackendCapabilities {
        BackendCapabilities {
            accelerators: vec![AcceleratorType::Vulkan],
    ...
    async fn load_model(
        ...
        let resp =
            self.client.get(&url).send().await.map_err(|e| {
                SynapseError::BackendError(format!("Cannot reach Vulkan server: {e}"))
            })?;

        if !resp.status().is_success() {
            return Err(SynapseError::BackendError(
                "Vulkan server returned error on /v1/models".into(),
            ));
        }

        let handle_id = format!("vulkan-{}", model_name.replace('/', "-"));
        info!(handle = %handle_id, model = %model_name, "Registered model with Vulkan backend");
    ...
    async fn unload_model(&self, handle: ModelHandle) -> Result<()> {
        ...
        if loaded.remove(&handle.0).is_some() {
            info!(handle = %handle.0, "Unregistered model from Vulkan backend");
            Ok(())
        } else {
    ...
    async fn infer(
        ...
        if !resp.status().is_success() {
            ...
            return Err(SynapseError::BackendError(format!("Vulkan error: {text}")));
        }
        ...
        tokio::spawn(async move {
            ...
            while let Some(chunk) = stream.next().await {
                ...
                let chunk = match chunk {
                    Ok(c) => c,
                    Err(e) => {
                        warn!("Vulkan stream error: {e}");
                        break;
                    }
                };
                buffer.push_str(&String::from_utf8_lossy(&chunk));
                if buffer.len() > MAX_BUFFER_SIZE {
                    warn!("Vulkan stream buffer exceeded {MAX_BUFFER_SIZE} bytes, aborting");
                    break;
                }
...
mod tests {
    ...
    #[test]
    fn backend_id() {
        let backend = VulkanBackend::new(None);
        assert_eq!(backend.id().0, "vulkan");
    }

    #[test]
    fn default_url() {
        let backend = VulkanBackend::new(None);
        assert_eq!(backend.base_url, "http://127.0.0.1:8003");
    }

    #[test]
    fn custom_url() {
        let backend = VulkanBackend::new(Some("http://gpu-server:8003".into()));
        assert_eq!(backend.base_url, "http://gpu-server:8003");
    }
    ...
    fn capabilities() {
        ...
        assert!(caps.accelerators.contains(&AcceleratorType::Vulkan));
        assert!(!caps.accelerators.contains(&AcceleratorType::Cuda));
        assert!(!caps.accelerators.contains(&AcceleratorType::Metal));
    ...
    async fn load_model_success() {
        ...
        let backend = VulkanBackend::new(Some(server.url()));
        ...
        let device = DeviceConfig {
            accelerator: AcceleratorType::Vulkan,
            device_ids: vec![0],
            memory_limit_mb: None,
        };

        let handle = backend.load_model(&manifest, &device).await.unwrap();
        assert_eq!(handle.0, "vulkan-org-test-model");
    ...
    async fn unload_model_success() {
        ...
        backend
            ...
            .write()
            .await
            .insert("vulkan-test".into(), "test-model".into());

        backend
            .unload_model(ModelHandle("vulkan-test".into()))
            .await
            .unwrap();
    ...
    async fn infer_success() {
        ...
        let resp = backend
            .infer(&ModelHandle("vulkan-test".into()), &req)
            .await
            .unwrap();
    ...
    async fn infer_server_error() {
        ...
        backend
            ...
            .write()
            .await
            .insert("vulkan-test".into(), "model".into());
    ...
    async fn health_check_server_down() {
        let backend = VulkanBackend::new(Some("http://127.0.0.1:1".into()));
        let healthy = backend.health_check().await.unwrap();
        assert!(!healthy);
    }
}
