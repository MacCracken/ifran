//! Intel OneAPI backend integration.
//!
//! HTTP client to a running IPEX-LLM or OpenVINO serving process for
//! Intel Arc / Data Center GPU Max. It exposes an OpenAI-compatible API
...
/// Intel OneAPI backend that proxies to a running IPEX-LLM / OpenVINO server.
...
impl OneApiBackend {
    pub fn new(base_url: Option<String>) -> Self {
        Self {
            base_url: base_url.unwrap_or_else(|| "http://127.0.0.1:8006".into()),
...
#[async_trait]
impl InferenceBackend for OneApiBackend {
    fn id(&self) -> BackendId {
        BackendId("oneapi".into())
    }

    fn capabilities(&self) -> BackendCapabilities {
        BackendCapabilities {
            accelerators: vec![AcceleratorType::OneApi],
    ...
    async fn load_model(
        ...
        let resp =
            self.client.get(&url).send().await.map_err(|e| {
                SynapseError::BackendError(format!("Cannot reach OneAPI server: {e}"))
            })?;

        if !resp.status().is_success() {
            return Err(SynapseError::BackendError(
                "OneAPI server returned error on /v1/models".into(),
            ));
        }

        let handle_id = format!("oneapi-{}", model_name.replace('/', "-"));
        info!(handle = %handle_id, model = %model_name, "Registered model with OneAPI backend");
    ...
    async fn unload_model(&self, handle: ModelHandle) -> Result<()> {
        ...
        if loaded.remove(&handle.0).is_some() {
            info!(handle = %handle.0, "Unregistered model from OneAPI backend");
            Ok(())
        } else {
    ...
    async fn infer(
        ...
        if !resp.status().is_success() {
            ...
            return Err(SynapseError::BackendError(format!("OneAPI error: {text}")));
        }
        ...
        tokio::spawn(async move {
            ...
            while let Some(chunk) = stream.next().await {
                ...
                let chunk = match chunk {
                    Ok(c) => c,
                    Err(e) => {
                        warn!("OneAPI stream error: {e}");
                        break;
                    }
                };
                buffer.push_str(&String::from_utf8_lossy(&chunk));
                if buffer.len() > MAX_BUFFER_SIZE {
                    warn!("OneAPI stream buffer exceeded {MAX_BUFFER_SIZE} bytes, aborting");
                    break;
                }
...
mod tests {
    ...
    #[test]
    fn backend_id() {
        let backend = OneApiBackend::new(None);
        assert_eq!(backend.id().0, "oneapi");
    }

    #[test]
    fn default_url() {
        let backend = OneApiBackend::new(None);
        assert_eq!(backend.base_url, "http://127.0.0.1:8006");
    }

    #[test]
    fn custom_url() {
        let backend = OneApiBackend::new(Some("http://intel-server:8006".into()));
        assert_eq!(backend.base_url, "http://intel-server:8006");
    }
    ...
    fn capabilities() {
        ...
        assert!(caps.accelerators.contains(&AcceleratorType::OneApi));
    ...
    async fn load_model_success() {
        ...
        let backend = OneApiBackend::new(Some(server.url()));
        ...
        let device = DeviceConfig {
            accelerator: AcceleratorType::OneApi,
            device_ids: vec![0],
            memory_limit_mb: None,
        };

        let handle = backend.load_model(&manifest, &device).await.unwrap();
        assert_eq!(handle.0, "oneapi-org-test-model");
    ...
    async fn unload_model_success() {
        ...
        backend
            ...
            .write()
            .await
            .insert("oneapi-test".into(), "test-model".into());

        backend
            .unload_model(ModelHandle("oneapi-test".into()))
            .await
            .unwrap();
    ...
    async fn infer_success() {
        ...
        let resp = backend
            .infer(&ModelHandle("oneapi-test".into()), &req)
            .await
            .unwrap();
    ...
    async fn infer_server_error() {
        ...
        backend
            ...
            .write()
            .await
            .insert("oneapi-test".into(), "model".into());
    ...
    async fn health_check_server_down() {
        let backend = OneApiBackend::new(Some("http://127.0.0.1:1".into()));
        let healthy = backend.health_check().await.unwrap();
        assert!(!healthy);
    }
}
