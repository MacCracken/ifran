//! Qualcomm Cloud AI backend integration.
//!
//! HTTP client to a running QAI Hub or QAIRT serving process for
//! Qualcomm Cloud AI 100. It exposes an OpenAI-compatible API that
//! this backend wraps.
...
/// Qualcomm Cloud AI backend that proxies to a running QAI Hub / QAIRT server.
pub struct QualcommBackend {
    /// Base URL of the serving process.
    base_url: String,
    /// HTTP client.
...
impl QualcommBackend {
    pub fn new(base_url: Option<String>) -> Self {
        Self {
            base_url: base_url.unwrap_or_else(|| "http://127.0.0.1:8007".into()),
...
#[async_trait]
impl InferenceBackend for QualcommBackend {
    fn id(&self) -> BackendId {
        BackendId("qualcomm".into())
    }

    fn capabilities(&self) -> BackendCapabilities {
        BackendCapabilities {
            accelerators: vec![AcceleratorType::QualcommAi],
    ...
    async fn load_model(
        ...
        let resp = self.client.get(&url).send().await.map_err(|e| {
            SynapseError::BackendError(format!("Cannot reach Qualcomm server: {e}"))
        })?;

        if !resp.status().is_success() {
            return Err(SynapseError::BackendError(
                "Qualcomm server returned error on /v1/models".into(),
            ));
        }

        let handle_id = format!("qualcomm-{}", model_name.replace('/', "-"));
        info!(handle = %handle_id, model = %model_name, "Registered model with Qualcomm backend");
    ...
    async fn unload_model(&self, handle: ModelHandle) -> Result<()> {
        ...
        if loaded.remove(&handle.0).is_some() {
            info!(handle = %handle.0, "Unregistered model from Qualcomm backend");
            Ok(())
        } else {
        ...
        tokio::spawn(async move {
            ...
            while let Some(chunk) = stream.next().await {
                ...
                let chunk = match chunk {
                    Ok(c) => c,
                    Err(e) => {
                        warn!("Qualcomm stream error: {e}");
                        break;
                    }
                };
                buffer.push_str(&String::from_utf8_lossy(&chunk));
                if buffer.len() > MAX_BUFFER_SIZE {
                    warn!("Qualcomm stream buffer exceeded {MAX_BUFFER_SIZE} bytes, aborting");
                    break;
                }
...
mod tests {
    ...
    #[test]
    fn backend_id() {
        let backend = QualcommBackend::new(None);
        assert_eq!(backend.id().0, "qualcomm");
    }

    #[test]
    fn default_url() {
        let backend = QualcommBackend::new(None);
        assert_eq!(backend.base_url, "http://127.0.0.1:8007");
    }

    #[test]
    fn custom_url() {
        let backend = QualcommBackend::new(Some("http://qc-server:8007".into()));
        assert_eq!(backend.base_url, "http://qc-server:8007");
    }
    ...
    fn capabilities() {
        ...
        assert_eq!(caps.max_context_length, Some(16384));
        assert!(caps.accelerators.contains(&AcceleratorType::QualcommAi));
    ...
    fn formats() {
        ...
        assert_eq!(backend.supported_formats().len(), 1);
    }
    ...
    async fn load_model_success() {
        ...
        let backend = QualcommBackend::new(Some(server.url()));
        ...
        let device = DeviceConfig {
            accelerator: AcceleratorType::QualcommAi,
            device_ids: vec![0],
            memory_limit_mb: None,
        };

        let handle = backend.load_model(&manifest, &device).await.unwrap();
        assert_eq!(handle.0, "qualcomm-org-test-model");
    ...
    async fn unload_model_success() {
        ...
        backend
            ...
            .write()
            .await
            .insert("qualcomm-test".into(), "test-model".into());

        backend
            .unload_model(ModelHandle("qualcomm-test".into()))
            .await
            .unwrap();
    ...
    async fn infer_success() {
        ...
        let resp = backend
            .infer(&ModelHandle("qualcomm-test".into()), &req)
            .await
            .unwrap();
    ...
    async fn infer_server_error() {
        ...
        backend
            ...
            .write()
            .await
            .insert("qualcomm-test".into(), "model".into());
    ...
    async fn health_check_server_down() {
        let backend = QualcommBackend::new(Some("http://127.0.0.1:1".into()));
        let healthy = backend.health_check().await.unwrap();
        assert!(!healthy);
    }
}
