#!/usr/bin/env python3
"""Direct Preference Optimization (DPO) training script for Synapse.
...
def parse_config():
    parser = argparse.ArgumentParser(description="Synapse DPO training")
...
def main():
    ...
    print(f"[synapse] Job {job_id}: Starting DPO training")

    hp = config["hyperparams"]
    base_model = config["base_model"]
    output_name = config.get("output_name", f"synapse-dpo-{job_id}")
    ...
    print(f"[synapse] Job {job_id}: DPO training started — {len(dataset)} preference pairs")
    trainer.train()

    # Save
    trainer.save_model(output_dir)
    tokenizer.save_pretrained(output_dir)
    print(f"[synapse] Job {job_id}: DPO training complete — saved to {output_dir}")
