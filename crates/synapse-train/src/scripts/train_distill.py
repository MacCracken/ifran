#!/usr/bin/env python3
"""Knowledge distillation training script for Synapse.
...
def parse_config():
    parser = argparse.ArgumentParser(description="Synapse knowledge distillation")
...
def main():
    ...
    print(f"[synapse] Job {job_id}: Starting knowledge distillation")

    hp = config["hyperparams"]
    base_model = config["base_model"]  # Student model
    teacher_model_name = config.get("teacher_model", None)
    output_name = config.get("output_name", f"synapse-distill-{job_id}")
    ...
    if teacher_model_name:
        print(f"[synapse] Loading teacher model: {teacher_model_name}")
    ...
    print(f"[synapse] Job {job_id}: Distillation started — {len(dataset)} samples, {teacher_info}")
    trainer.train()

    # Save
    trainer.save_model(output_dir)
    tokenizer.save_pretrained(output_dir)
    print(f"[synapse] Job {job_id}: Distillation complete — saved to {output_dir}")
