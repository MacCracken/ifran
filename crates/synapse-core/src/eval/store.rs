...
use synapse_types::eval::{BenchmarkKind, EvalResult, EvalRunId};
use uuid::Uuid;
...
impl EvalStore {
    ...
    pub fn insert(&self, result: &EvalResult) -> Result<()> {
        self.conn
            .execute(
                "INSERT INTO eval_results (run_id, model_name, benchmark, score, details,
                    samples_evaluated, duration_secs, evaluated_at)
    ...
    pub fn get_run(&self, run_id: EvalRunId) -> Result<Vec<EvalResult>> {
        let mut stmt = self
            .conn
            .prepare("SELECT * FROM eval_results WHERE run_id = ?1")
            .map_err(|e| SynapseError::StorageError(e.to_string()))?;

        let results = stmt
            .query_map(params![run_id.to_string()], row_to_eval_result)
            .map_err(|e| SynapseError::StorageError(e.to_string()))?
    ...
    pub fn get_by_model(&self, model_name: &str) -> Result<Vec<EvalResult>> {
        let mut stmt = self
            .conn
            .prepare("SELECT * FROM eval_results WHERE model_name = ?1 ORDER BY evaluated_at DESC")
            .map_err(|e| SynapseError::StorageError(e.to_string()))?;

        let results = stmt
            .query_map(params![model_name], row_to_eval_result)
            .map_err(|e| SynapseError::StorageError(e.to_string()))?
            .collect::<std::result::Result<Vec<_>, _>>()
            .map_err(|e| SynapseError::StorageError(e.to_string()))?;
    ...
    pub fn list(&self) -> Result<Vec<EvalResult>> {
        let mut stmt = self
            .conn
            .prepare("SELECT * FROM eval_results ORDER BY evaluated_at DESC")
            .map_err(|e| SynapseError::StorageError(e.to_string()))?;

        let results = stmt
            .query_map([], row_to_eval_result)
            .map_err(|e| SynapseError::StorageError(e.to_string()))?
            .collect::<std::result::Result<Vec<_>, _>>()
            .map_err(|e| SynapseError::StorageError(e.to_string()))?;
...
mod tests {
    ...
    fn insert_and_get_by_model() {
        ...
        let fetched = store.get_by_model("llama-3.1-8b").unwrap();
