...
impl OciClient {
    ...
    pub async fn get_manifest(&self, name: &str, tag: &str) -> Result<OciManifest> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| SynapseError::DownloadError(format!("OCI request failed: {e}")))?;

        if resp.status() == reqwest::StatusCode::NOT_FOUND {
            return Err(SynapseError::ModelNotFound(format!("{name}:{tag}")));
        }
        ...
        resp.json::<OciManifest>()
            .await
            .map_err(|e| SynapseError::DownloadError(format!("Failed to parse OCI manifest: {e}")))
    }
