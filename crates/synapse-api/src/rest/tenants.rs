...
//! Protected by `SYNAPSE_ADMIN_KEY` environment variable.
...
pub async fn disable_tenant(
    ...
    let tenant_id = synapse_types::TenantId(id);
    let store = store.lock().await;
    store
...
/// Admin auth middleware — validates `SYNAPSE_ADMIN_KEY` Bearer token.
pub async fn require_admin_auth(
    req: axum::extract::Request,
    next: axum::middleware::Next,
) -> Result<axum::response::Response, StatusCode> {
    let admin_key = std::env::var("SYNAPSE_ADMIN_KEY")
        .ok()
        .filter(|k| !k.is_empty())
