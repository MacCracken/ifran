...
//!
//! Implements the `SynapseService` trait generated from `proto/synapse.proto`.
//! Mirrors the REST API logic but over gRPC for lower-latency programmatic access.

use crate::state::AppState;
use std::pin::Pin;
use synapse_types::synapse_proto::synapse_service_server::SynapseService;
use synapse_types::synapse_proto::{
...
#[tonic::async_trait]
impl SynapseService for SynapseGrpcService {
    // -----------------------------------------------------------------------
    // GetStatus
    // -----------------------------------------------------------------------
    ...
    async fn get_status(
        ...
        let gpus = synapse_core::hardware::detect::detect()
            .map(|hw| {
                hw.gpus
                    .iter()
                    .map(|g| GpuInfo {
    ...
    async fn list_models(
        ...
        let models = db
            .list(&synapse_types::TenantId::default_tenant())
...
mod tests {
    ...
    async fn get_status_returns_empty_when_no_models() {
        ...
        let svc = SynapseGrpcService::new(state);
