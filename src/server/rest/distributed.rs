...
use crate::training_events::TrainingEvent;
use crate::types::TenantId;
use crate::types::distributed::*;
use crate::types::training::{TrainingJobConfig, TrainingStatus};
...
pub struct AutoPlaceRequest {
    ...
    pub policy: crate::types::distributed::PlacementPolicyKind,
    /// Optional: GPUs per worker (default: 1).
    #[serde(default = "default_gpus_per_worker")]
    pub gpus_per_worker: u32,
}

fn default_placement_policy() -> crate::types::distributed::PlacementPolicyKind {
    crate::types::distributed::PlacementPolicyKind::GpuAffinity
}
...
pub async fn aggregate(
    ...
    use crate::train::distributed::aggregator::{
...
pub async fn auto_place(
    ...
    let online_nodes: Vec<_> = fleet_nodes
        .iter()
        .filter(|n| n.health == crate::fleet::manager::NodeHealth::Online)
        .collect();
    ...
    let node_resources: Vec<crate::train::distributed::placement::NodeResources> = online_nodes
        .iter()
        .map(|n| {
            let available_gpu_ids: Vec<u32> = (0..n.gpu_info.gpu_count as u32).collect();
            crate::train::distributed::placement::NodeResources {
                node_id: n.id.clone(),
    ...
    let policy = crate::train::distributed::placement::policy_from_kind(req.policy);
...
mod tests {
    use super::*;
    use crate::config::*;
    use crate::server::rest::pagination::PaginationQuery;
    use crate::server::state::AppState;
    use crate::types::TenantId;
