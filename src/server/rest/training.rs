...
pub struct MetricsResponse {
    ...
    pub current_epoch: f32,
    pub progress_percent: f64,
    pub checkpoints: Vec<crate::types::training::CheckpointInfo>,
}
...
async fn load_checkpoints(
    state: &AppState,
    id: TrainingJobId,
) -> Result<Vec<crate::types::training::CheckpointInfo>, ApiErrorResponse> {
    let checkpoints_dir = state.config.training.checkpoints_dir.clone();
    tokio::task::spawn_blocking(move || {
        crate::train::checkpoint::store::CheckpointStore::new(checkpoints_dir).list(id)
    })
    .await
...
pub async fn list_checkpoints(
    ...
    Extension(tenant_id): Extension<TenantId>,
    Path(id): Path<TrainingJobId>,
) -> Result<Json<Vec<crate::types::training::CheckpointInfo>>, ApiErrorResponse> {
    state
        .job_manager
        .get_job(id, &tenant_id)
        .await
...
mod tests {
    ...
    async fn job_to_response_conversion() {
        ...
        let mut job =
            crate::train::job::status::JobState::new(id, TenantId::default_tenant(), config, 1000);
        job.start();
    ...
    async fn stream_job_returns_sse() {
        ...
        let app = crate::server::rest::router::build(state);
        let response = app
            .oneshot(
                Request::builder()
