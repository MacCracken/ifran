...
pub async fn execute(
    ...
    let dataset_path = dataset.ok_or_else(|| {
        crate::types::IfranError::EvalError(
    ...
    let http_client = crate::pull::downloader::build_client()?;
    let infer_fn = |prompt: String| {
        ...
        async move {
            let resp = client
                ...
                .await
                .map_err(|e| {
                    crate::types::IfranError::EvalError(format!(
                        "Inference request failed (is the API server running?): {e}"
                    ))
                })?;

            if !resp.status().is_success() {
                return Err(crate::types::IfranError::EvalError(format!(
            ...
            let body: serde_json::Value = resp.json().await.map_err(|e| {
                crate::types::IfranError::EvalError(format!("Invalid inference response: {e}"))
            })?;

            body["text"].as_str().map(String::from).ok_or_else(|| {
                crate::types::IfranError::EvalError("No 'text' field in inference response".into())
            })
        }
    };
...
fn parse_benchmark(s: &str) -> crate::types::error::Result<BenchmarkKind> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(crate::types::IfranError::EvalError(format!(
