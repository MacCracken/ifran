...
impl IfranConfig {
    ...
    pub fn load(path: &std::path::Path) -> crate::types::error::Result<Self> {
        let content = std::fs::read_to_string(path)
            .map_err(|e| crate::types::IfranError::ConfigError(e.to_string()))?;
        toml::from_str(&content).map_err(|e| crate::types::IfranError::ConfigError(e.to_string()))
    }
