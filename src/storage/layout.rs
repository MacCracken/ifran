...
//!
//! Manages the directory structure under `~/.ifran/` (or a configured root).
//! Each model gets its own directory named by a slug derived from the model name.
//!
//! ```text
//! ~/.ifran/
//! ├── ifran.db          # SQLite catalog
...
mod tests {
    ...
    fn paths_are_correct() {
        let layout = StorageLayout::new("/tmp/ifran-test-layout");
        assert_eq!(
            layout.database_path(),
            PathBuf::from("/tmp/ifran-test-layout/ifran.db")
        );
        assert_eq!(
            layout.models_dir(),
            PathBuf::from("/tmp/ifran-test-layout/models")
        );
        assert_eq!(
            layout.model_dir("llama-8b-q4km"),
            PathBuf::from("/tmp/ifran-test-layout/models/llama-8b-q4km")
        );
        assert_eq!(
            layout.model_metadata("llama-8b-q4km"),
            PathBuf::from("/tmp/ifran-test-layout/models/llama-8b-q4km/metadata.json")
        );
    }
    ...
    fn ensure_dirs_creates_structure() {
        ...
        let layout = StorageLayout::new(tmp.path().join("ifran"));
    ...
    fn config_path() {
        let layout = StorageLayout::new("/root");
        assert_eq!(layout.config_path(), PathBuf::from("/root/ifran.toml"));
    }
