/// Ifran CLI - command-line interface for managing models and inference.
use clap::{Parser, Subcommand};

use ifran::cli::commands;

#[derive(Parser)]
#[command(name = "ifran", about = "Local LLM inference and training platform")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
mod tests {
    ...
    #[test]
    fn cli_pull_basic() {
        let cli = Cli::try_parse_from(["ifran", "pull", "meta-llama/Llama-3.1-8B"]).unwrap();
        match cli.command {
    ...
    #[test]
    fn cli_list() {
        let cli = Cli::try_parse_from(["ifran", "list"]).unwrap();
    ...
    #[test]
    fn cli_status() {
        let cli = Cli::try_parse_from(["ifran", "status"]).unwrap();
        assert!(matches!(cli.command, Commands::Status));
    }

    #[test]
    fn cli_remove() {
        let cli = Cli::try_parse_from(["ifran", "remove", "llama-7b"]).unwrap();
        match cli.command {
    ...
    fn cli_remove_with_yes() {
        let cli = Cli::try_parse_from(["ifran", "remove", "llama-7b", "-y"]).unwrap();
        match cli.command {
    ...
    fn cli_eval_defaults() {
        let cli = Cli::try_parse_from(["ifran", "eval", "llama-7b"]).unwrap();
        match cli.command {
            Commands::Eval {
                model,
                benchmark,
    ...
    fn cli_marketplace_search() {
        let cli = Cli::try_parse_from(["ifran", "marketplace", "search", "llama"]).unwrap();
        match cli.command {
    ...
    fn cli_marketplace_publish() {
        let cli = Cli::try_parse_from(["ifran", "marketplace", "publish", "my-model"]).unwrap();
        match cli.command {
            Commands::Marketplace {
    ...
    fn cli_no_args_fails() {
        assert!(Cli::try_parse_from(["ifran"]).is_err());
    }

    #[test]
    fn cli_unknown_command_fails() {
        assert!(Cli::try_parse_from(["ifran", "nonexistent"]).is_err());
    }

    #[test]
    fn cli_experiment_run() {
        let cli = Cli::try_parse_from(["ifran", "experiment", "run", "experiment.toml"]).unwrap();
        match cli.command {
            Commands::Experiment {
    ...
    fn cli_experiment_list() {
        let cli = Cli::try_parse_from(["ifran", "experiment", "list"]).unwrap();
        assert!(matches!(
            cli.command,
            Commands::Experiment {
    ...
    fn cli_experiment_status_no_id() {
        let cli = Cli::try_parse_from(["ifran", "experiment", "status"]).unwrap();
        match cli.command {
            Commands::Experiment {
    ...
    fn cli_experiment_status_with_id() {
        let cli = Cli::try_parse_from(["ifran", "experiment", "status", "abc-123"]).unwrap();
        match cli.command {
            Commands::Experiment {
                action: ExperimentAction::Status { id },
    ...
    fn cli_experiment_leaderboard_default_limit() {
        let cli = Cli::try_parse_from(["ifran", "experiment", "leaderboard", "exp-1"]).unwrap();
        match cli.command {
            Commands::Experiment {
    ...
    fn cli_experiment_stop() {
        let cli = Cli::try_parse_from(["ifran", "experiment", "stop", "exp-1"]).unwrap();
        match cli.command {
            Commands::Experiment {
    ...
    fn cli_marketplace_unpublish() {
        let cli = Cli::try_parse_from(["ifran", "marketplace", "unpublish", "my-model"]).unwrap();
        match cli.command {
            Commands::Marketplace {
    ...
    fn cli_marketplace_search_no_query() {
        let cli = Cli::try_parse_from(["ifran", "marketplace", "search"]).unwrap();
        match cli.command {
            Commands::Marketplace {
                action: MarketplaceAction::Search { query },
