...
use crate::backends::{InferenceBackend, ModelHandle, hash_text_to_embedding};
use crate::types::rag::{RagPipelineConfig, RagPipelineId, RagQuery, RagSource};
...
use crate::server::middleware::validation::validate_filename;
...
async fn resolve_embedding_backend(
    ...
    let backend = state.backends.get(&crate::types::backend::BackendId(
...
pub async fn ingest_document(
    ...
    let pipeline = crate::rag::pipeline::RagPipeline::new(store, id, config);
    let doc = pipeline
...
pub async fn query(
    ...
    let pipeline = crate::rag::pipeline::RagPipeline::new(store, req.pipeline_id, config);
