...
//! in bridge.proto. Ifran uses this to request GPU allocations, report
//! progress, and register completed models with SY.

use crate::bridge::protocol::{Capabilities, ConnectionState, ProtocolConfig};
...
impl BridgeClient {
    ...
    pub async fn reconnect(&self) -> crate::types::error::Result<()> {
        ...
        if *count >= self.config.max_reconnect_attempts {
            ...
            *self.state.write().await = ConnectionState::Degraded;
            return Err(crate::types::IfranError::BridgeError(
    ...
    async fn get_client(&self) -> crate::types::error::Result<Option<YeomanBridgeClient<Channel>>> {
        Ok(self.grpc_client.read().await.clone())
    }

    /// Send a capabilities announcement to SY.
    pub async fn announce(&self, capabilities: Capabilities) -> crate::types::error::Result<()> {
        info!(
        ...
        if let Some(mut client) = self.get_client().await? {
            ...
            client
                ...
                .await
                .map_err(|e| {
                    crate::types::IfranError::BridgeError(format!(
    ...
    pub async fn request_gpu(
        ...
        memory_mb: u64,
        count: u32,
    ) -> crate::types::error::Result<Vec<u32>> {
    ...
    pub async fn report_progress(
        ...
        if let Some(mut client) = self.get_client().await? {
            ...
            client
                ...
                .await
                .map_err(|e| {
                    crate::types::IfranError::BridgeError(format!("ReportProgress RPC failed: {e}"))
                })?;
        } else {
    ...
    pub async fn request_scale_out(
        ...
        additional: u32,
        reason: &str,
    ) -> crate::types::error::Result<ScaleResponse> {
        ...
        if let Some(mut client) = self.get_client().await? {
            ...
            let response = client.request_scale_out(request).await.map_err(|e| {
                crate::types::IfranError::BridgeError(format!("RequestScaleOut RPC failed: {e}"))
            })?;
