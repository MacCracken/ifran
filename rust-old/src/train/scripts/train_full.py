#!/usr/bin/env python3
"""Full parameter fine-tuning script for Ifran.
...
def parse_config():
    parser = argparse.ArgumentParser(description="Ifran full fine-tuning")
...
def main():
    ...
    print(f"[ifran] Job {job_id}: Starting full fine-tuning")

    hp = config["hyperparams"]
    base_model = config["base_model"]
    output_name = config.get("output_name", f"ifran-full-{job_id}")
    ...
    print(f"[ifran] Total parameters: {total_params:,}")
    print(f"[ifran] Trainable parameters: {trainable_params:,}")
    ...
    print(f"[ifran] Job {job_id}: Full fine-tuning started — {len(dataset)} samples, {hp.get('epochs', 3)} epochs")
    trainer.train()

    # Save
    trainer.save_model(output_dir)
    tokenizer.save_pretrained(output_dir)
    print(f"[ifran] Job {job_id}: Full fine-tuning complete — saved to {output_dir}")
